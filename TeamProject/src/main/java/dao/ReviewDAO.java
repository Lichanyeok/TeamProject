...
//			e.printStackTrace();
	...
	public ArrayList<ReviewBean> getByStoreList() {
		...
		try {
			sql = "SELECT * FROM review GROUP BY rev_store ORDER BY rev_num DESC LIMIT 10"; // 매장 중복 없이 최근 10개 리뷰 불러오기
			pstmt = con.prepareStatement(sql);
    ...
    public ArrayList<ReviewBean> selectByStoreScore() {
    	ArrayList<ReviewBean> ByStoreScore = null; // 게시물을 저장할 객체 생성
		...
		try {
			sql = "SELECT rev_store, COUNT(rev_score) AS rev_count, AVG(rev_score) AS rev_score FROM review GROUP BY rev_store ORDER BY rev_num DESC LIMIT 10";
			pstmt = con.prepareStatement(sql);
			...
			// 모든 레코드를 저장할 List 객체(ArrayList) 생성
			ByStoreScore = new ArrayList<ReviewBean>();
			
			// While 문을 
			while(rs.next()) {
				// ReviewBean 객체를 생성하여 1개 레코드 정보를 ReviewBean 객체에 저장
				// => 총 별점 평균, 매장명만 필요
				ReviewBean rb = new ReviewBean();
				rb.setRev_score(rs.getFloat("rev_score")); // 매장 별 리뷰 평균 저장
				rb.setListCount(rs.getInt("rev_count")); // 매장 별 총 리뷰 수 저장
