...
public class PublishAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//target 으로 project_member 테이블 조회해서 id 가져오기
		...
		String name =""; //쿠폰 값을 확인하여 초기화할 변수들 선언
		String value="";
		int price = 0;
		//가져온 쿠폰의 번호를 확인하여 벨류 지정
		switch (coupon_name) {
		...
		case 3: value = "1000원 할인";break;
		case 4: value = "1000원 할인";break;
		}
		switch (coupon_price) {
		...
		Set<String> arrRandom = new HashSet<String>(); // 쿠폰번호의 중복을 막기 위해 Set 객체 생성
		while(arrRandom.size()<list.size()) {
			arrRandom.add(BoardDAO.getRandomStr(8));
		}
		System.out.println("난수 갯수" + arrRandom.size() + " 아이디 갯수 " + list.size());
