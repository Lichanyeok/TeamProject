...
<div class="reserve_list_wrap">
    ...
    <table>
        ...
        <table>
            ...
            <%if (isGone) {%>
            <td><%=reserveList.get(i).getReserve_date()%>
            </td>
            <%} else { %>
            <td>기간이 지난 목록입니다</td>
            <%} %>
            <td><%=reserveList.get(i).getReserve_time()%>
            </td>
            <td>
            ...
            <td id="rev_btn"><a>
                <button onclick="doPost(<%=i %>)">리뷰쓰기</button>
            </a></td>
            <%} else { %>
            <td><a href="Review.re?nickName=<%=session.getAttribute("sNn") %>">
                <button>작성한 리뷰 보러가기</button>
            </a></td>
            <%} %>
            </tbody>
            <%} %>
